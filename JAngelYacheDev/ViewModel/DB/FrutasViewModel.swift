...
class FrutasViewModel{
    ...
    func Update(fruta : Frutas)->Result{
        var result = Result()
        let context = DB.init()
        let query = "UPDATE Frutas SET(Clave, ArrayPrecios, Nombre, FechaModificacion)  = (?, ?, ?, ?) WHERE (IdFruta = \(fruta.Id))"
    ...
    func Add(frutas : Frutas)->Result{
        ...
        let context = DB.init()
        let query = "Insert INTO Frutas(Clave, ArrayPrecios, Nombre, FechaRegistro, FechaModificacion) VALUES(?,?,?,?,?)"
        var statement : OpaquePointer? = nil
        do{
            if try sqlite3_prepare_v2(context.db, query, -1, &statement, nil) == SQLITE_OK{
    ...
    func getById(IdFruta : Int)->Result{
        var result = Result()
        let context = DB.init()
        let query = "SELECT IdFruta, Clave, Nombre, ArrayPrecios, FechaRegistro, FechaModificacion FROM Frutas WHERE (IdFruta = \(IdFruta))"
        var statement : OpaquePointer? = nil
        do{
            if try sqlite3_prepare_v2(context.db, query, -1, &statement, nil) == SQLITE_OK{
                result.Object
    ...
    func getAll()-> Result{
        ...
        var result = Result()
        let context = DB.init()
        let query = "SELECT  IdFruta, Clave, ArrayPrecios, Nombre, FechaRegistro, FechaModificacion FROM Frutas"
        var statement : OpaquePointer? = nil
        do{
            if try sqlite3_prepare_v2(context.db, query, -1, &statement, nil) == SQLITE_OK{
                result.Objects = []
